...
from cycles.SC1R import run_SC1R_cycle
...
def run_sample(row):
    ...
    try:
        log_A_evap, log_A_cond, log_A_recup = run_SC1R_cycle(sample)
    except Exception as e:
        print(f"Simulation failed: {e}")
        log_A_evap, log_A_cond, log_A_recup = np.nan, np.nan, np.nan
    
    return log_A_evap, log_A_cond, log_A_recup
...
def generate_samples(cycle_name, n_samples, seed=42, n_workers="normal"):
    ...
    df_new["log_A_evap"], df_new["log_A_cond"], df_new["log_A_recup"] = zip(*results)
    df_new.dropna(subset=["log_A_evap", "log_A_cond", "log_A_recup"], inplace=True)
