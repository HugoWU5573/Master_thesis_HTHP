...
def run_sample(row):
    ...
    try:
        log_A_evap_LT, log_A_evap_MT, log_A_GasCooler = run_TC2_cycle(sample)
    except Exception as e:
        print(f"Simulation failed: {e}")
        log_A_evap_LT, log_A_evap_MT, log_A_GasCooler = np.nan, np.nan, np.nan
    
    return log_A_evap_LT, log_A_evap_MT, log_A_GasCooler
...
def generate_samples(cycle_name, n_samples, seed=42, n_workers="normal"):
    ...
    df_new["log_A_evap_LT"], df_new["log_A_evap_MT"], df_new["log_A_GasCooler"] = zip(*results)
    df_new.dropna(subset=["log_A_evap_LT", "log_A_evap_MT", "log_A_GasCooler"], inplace=True)
...
if __name__ == "__main__":
    ...
    if post_process:
        ...
        print("-" * 128)
        print(f"| {'Sample Size':<12} | {'Mean Nb plates Evap MT':<25} | {'Std Nb plates Evap MT':<25} | {'':<25} | {'':<25} |")
        print("-" * 128)
        for i, size in enumerate(samples_sizes):
            print(f"| {size:<12} | {mean_nb_plates_evap_MT[i]:<25.2f} | {std_nb_plates_evap_MT[i]:<25.2f} | {'':<25} | {'':<25} |")
        print("-" * 128)
