...
def run_sample(row):
    ...
    try:
        log_A_evap, log_A_GasCooler, log_A_recup = run_TC1R_cycle(sample)
    except Exception as e:
        print(f"Simulation failed: {e}")
        log_A_evap, log_A_GasCooler, log_A_recup = np.nan, np.nan, np.nan
    
    return log_A_evap, log_A_GasCooler, log_A_recup
...
def generate_samples(cycle_name, n_samples, seed=42, n_workers="normal"):
    ...
    df_new["log_A_evap"], df_new["log_A_GasCooler"], df_new["log_A_recup"] = zip(*results)
    df_new.dropna(subset=["log_A_evap", "log_A_GasCooler", "log_A_recup"], inplace=True)
...
if __name__ == "__main__":
    ...
    if post_process:
        ...
        log_A_recup = df["log_A_recup"].values
        phi = df["phi"].values
        ...
        Nb_plates_recup = np.zeros(len(log_A_recup))
        ...
        for i in range(len(log_A_evap)):
            ...
            Nb_plates_recup[i] = logA_to_Nplates(log_A_recup[i], phi[i], model="ACK18")
        ...
        mean_nb_plates_recup = np.zeros(len(samples_sizes))
        std_nb_plates_recup = np.zeros(len(samples_sizes))
        ...
        for i, size in enumerate(samples_sizes):
            ...
            mean_nb_plates_recup[i] = np.mean(Nb_plates_recup[:size])
            std_nb_plates_recup[i] = np.std(Nb_plates_recup[:size])

        # Print a table with the different statistics depending on the sample size
        print("-" * 128)
        print(f"| {'Sample Size':<12} | {'Mean Nb plates Evap':<25} | {'Std Nb plates Evap':<25} | {'Mean Nb plates GasC.':<25} | {'Std Nb plates GasC.':<25} |")
        print("-" * 128)
        for i, size in enumerate(samples_sizes):
            print(f"| {size:<12} | {mean_nb_plates_evap[i]:<25.2f} | {std_nb_plates_evap[i]:<25.2f} | {mean_nb_plates_GasCooler[i]:<25.2f} | {std_nb_plates_GasCooler[i]:<25.2f} |")
        print("-" * 128)
        print(f"| {'Sample Size':<12} | {'Mean Nb plates Recup':<25} | {'Std Nb plates Recup':<25} | {'':<25} | {'':<25} |")
        print("-" * 128)
        for i, size in enumerate(samples_sizes):
            print(f"| {size:<12} | {mean_nb_plates_recup[i]:<25.2f} | {std_nb_plates_recup[i]:<25.2f} | {'':<25} | {'':<25} |")
        print("-" * 128)
        ...
        # Plot the empirical CDFs for the recuperator
        name = "Recup_MT_CDF"
        Nb_plates = Nb_plates_recup
        ...
        # Plot the histograms of the PDfs for the recuperator
        name = "Recup_MT_PDF"
