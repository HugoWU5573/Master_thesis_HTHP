...
from code.cycles.thermodynamic.SC2 import run_SC2_cycle
...
def run_sample(row):
    ...
    try:
        log_A_evap_LT, log_A_evap_MT, log_A_cond = run_SC2_cycle(sample)
    except Exception as e:
        print(f"Simulation failed: {e}")
        log_A_evap_LT, log_A_evap_MT, log_A_cond = np.nan, np.nan, np.nan
    
    return log_A_evap_LT, log_A_evap_MT, log_A_cond
...
def generate_samples(cycle_name, n_samples, seed=42, n_workers="normal"):
    ...
    df_new["log_A_evap_LT"], df_new["log_A_evap_MT"], df_new["log_A_cond"] = zip(*results)
    df_new.dropna(subset=["log_A_evap_LT", "log_A_evap_MT", "log_A_cond"], inplace=True)
