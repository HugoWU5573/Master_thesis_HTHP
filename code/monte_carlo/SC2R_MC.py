...
from code.cycles.thermodynamic.SC2R import run_SC2R_cycle
...
def run_sample(row):
    ...
    try:
        log_A_evap_LT, log_A_evap_MT, log_A_cond, log_A_recup_LT, log_A_recup_MT = run_SC2R_cycle(sample)
    except Exception as e:
        print(f"Simulation failed: {e}")
        log_A_evap_LT, log_A_evap_MT, log_A_cond, log_A_recup_LT, log_A_recup_MT = np.nan, np.nan, np.nan, np.nan, np.nan
    
    return log_A_evap_LT, log_A_evap_MT, log_A_cond, log_A_recup_LT, log_A_recup_MT
...
def generate_samples(cycle_name, n_samples, seed=42, n_workers="normal"):
    ...
    df_new["log_A_evap_LT"], df_new["log_A_evap_MT"], df_new["log_A_cond"], df_new["log_A_recup_LT"], df_new["log_A_recup_MT"] = zip(*results)
    df_new.dropna(subset=["log_A_evap_LT", "log_A_evap_MT", "log_A_cond", "log_A_recup_LT", "log_A_recup_MT"], inplace=True)
...
if __name__ == "__main__":
    ...
    if post_process:
        ...
        print("-" * 128)
        print(f"| {'Sample Size':<12} | {'Mean Nb plates Evap LT':<25} | {'Std Nb plates Evap LT':<25} | {'Mean Nb plates Cond':<25} | {'Std Nb plates Cond':<25} |")
        print("-" * 128)
        for i, size in enumerate(samples_sizes):
            print(f"| {size:<12} | {mean_nb_plates_evap_LT[i]:<25.2f} | {std_nb_plates_evap_LT[i]:<25.2f} | {mean_nb_plates_cond[i]:<25.2f} | {std_nb_plates_cond[i]:<25.2f} |")
        print("-" * 128)
