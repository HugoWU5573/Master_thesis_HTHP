...
from cycles.TC1 import run_TC1_cycle
...
def run_sample(row):
    ...
    try:
        log_A_evap, log_A_GasCooler = run_TC1_cycle(sample)
    except Exception as e:
        print(f"Simulation failed: {e}")
        log_A_evap, log_A_GasCooler = np.nan, np.nan
    
    return log_A_evap, log_A_GasCooler
...
def generate_samples(cycle_name, n_samples, seed=42, n_workers="normal"):
    ...
    df_new["log_A_evap"], df_new["log_A_GasCooler"] = zip(*results)
    df_new.dropna(subset=["log_A_evap", "log_A_GasCooler"], inplace=True)
