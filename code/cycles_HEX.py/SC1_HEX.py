...
for i in range(len(T1_prime_range)):
    for j in range(len(T4_prime_range)):
        ...
        Evaporator_areas[i, j] = SC1.Evaporator.A
        Condenser_areas[i, j] = SC1.Condenser.A
...
output_file = Path(__file__).parent.parent / "Figures" / SC1.name / f"{SC1.name}_statistics.txt"
