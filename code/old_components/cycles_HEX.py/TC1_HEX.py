...
T_7_max = 350                      # Maximum outlet temperature of the gas cooler [K]
...
T7_last = (T5_prime_min + T_pinch + T_7_max) / 2
...
for i in range(len(T3_prime_range)):
    for j in range(len(T5_prime_range)):
        ...
        optimization_vars_guess = [T_sup_last, T7_last]
        ...
        COP_values[i, j] = TC1.COP

                # Warning message if the highest pressure of the cycle is lower than the critical pressure (42.5 bar for R290)
        if TC1.state_5.p < HEOS_working_fluid.p_critical() :
            print("WARNING : The highest pressure of the cycle is lower than the critical pressure of the working fluid ({:.2f} bar < {:.2f} bar)".format(TC1.state_5.p / 1e5, HEOS_working_fluid.p_critical() / 1e5))

        # Warning message if the highest pressure of the cycle exceeds 50 bar
        if TC1.state_5.p > 5e6 :
            print("WARNING : The highest pressure of the cycle exceeds 50 bar ({:.2f} bar) -> not too problematic for the current analysis.".format(TC1.state_5.p / 1e5))
        ...
        Evaporator_areas[i, j] = TC1.Evaporator.A
        GasCooler_areas[i, j] = TC1.GasCooler.A
...
output_file = Path(__file__).parent.parent / "Figures" / TC1.name / f"{TC1.name}_statistics.txt"
