...
def run_Full_Model(compressors_frequencies, valves_openings, name="", transcritical=False, recuperator=False, save_results=False):
    ...
    def iterative_process(args, results=False, tol=5e-3):
        ...
        try :
            ...
            while ((np.abs((h1_old - h1_new)/h1_new) > tol) or (np.abs((p1_old - p1_new)/p1_new) > tol) 
                ...
                elif (iteration_counter == 1 and not transcritical) :
                ...
                elif (iteration_counter == 1 and transcritical) :
                    # Initial guess for state 2 (superheated state at the same pressure as state 1)
                    cycle.state_2 = State(HEOS_working_fluid, p=(cycle.state_1.p), T=cycle.state_1.T+50)
                else :
                ...
                if not transcritical :
                    T5, P_LP, mdot_working_fluid = Comp_LP.Solve(cycle.state_2, p5, N_LP)
                    cycle.state_5 = State(HEOS_working_fluid, T=T5, p=p5)
                else :
                    T5, P_HP, mdot_working_fluid = Comp_HP.Solve(cycle.state_2, p5, N_HP)
                    cycle.state_5 = State(HEOS_working_fluid, T=T5, p=p5)
            ...
            if results:
                ...
                if transcritical :
                    cycle.transforms.append(Transform('comp', '2', '5', Comp_HP))
                    cycle.mdot_wf_top = mdot_working_fluid
                    cycle.mdot_wf_bottom = 0
                    cycle.P_comp_top = P_HP
                    cycle.P_comp_bottom = 0
                else :
                    cycle.transforms.append(Transform('comp', '2', '5', Comp_LP))
                    cycle.mdot_wf_top = 0
                ...
                if recuperator :
                    Reccuperator_LT._plot(save = save_results, name_cycle=cycle.name)
...
if __name__ == "__main__":
    ...
    if run_SC1 :
        ...
        valves_openings = [37, 50]             # Openings of the valves [-]

        run_Full_Model(compressors_frequencies, valves_openings, name="SC1", transcritical=False, recuperator=False, save_results=save_results)
    ...
    if run_SC1R :
        ...
        run_Full_Model(compressors_frequencies, valves_openings, name="SC1R", transcritical=False, recuperator=True, save_results=save_results)

    # TC1 cycle

    if run_TC1 :
        compressors_frequencies = [50, 50]     # Frequencies of the compressors [Hz]
        valves_openings = [12, 50]             # Openings of the valves [-]

        run_Full_Model(compressors_frequencies, valves_openings, name="TC1", transcritical=True, recuperator=False, save_results=save_results)

    # TC1R cycle

    if run_TC1R :
        compressors_frequencies = [50, 50]     # Frequencies of the compressors [Hz]
        valves_openings = [9, 50]             # Openings of the valves [-]

        run_Full_Model(compressors_frequencies, valves_openings, name="TC1R", transcritical=True, recuperator=True, save_results=save_results)
    ...
    for i in range(len(valves_openings_list)):
        ...
        name = "SC1_" + str(int(valves_openings[0]))
        run_Full_Model(compressors_frequencies, valves_openings, name=name,transcritical=False ,recuperator=False, save_results=False)
