...
def evaluate(x_in, params = []):
    ...
    try :
        log_A_evap_LT, log_A_evap_MT, log_A_GasCooler = run_TC2_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False)
        
        return float(log_A_evap_LT), float(log_A_evap_MT), float(log_A_GasCooler)
