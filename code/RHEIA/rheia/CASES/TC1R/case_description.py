...
def evaluate(x_in, params = []):
    ...
    try :
        log_A_evap, log_A_GasCooler, log_A_recup = run_TC1R_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False)
        
        return float(log_A_evap), float(log_A_GasCooler), float(log_A_recup)
