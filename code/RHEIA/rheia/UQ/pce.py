    ...
    def evaluate(self, eval_func, params):
        ...
        if self.my_data.inputs['n jobs'] == 1:
            ...
            for index, sample in enumerate(eval_dict):
                y = eval_func((index + len(self.x_prev), sample), params=params)

                # store the model output only if it isn't a nan (i.e. if the model evaluation was successful)
                if np.isnan(y).any():
                    continue
                ...
                with open(self.my_data.filename_samples, 'a+') as file:
                    if isinstance(y, float):
                        line = list(samples[index]) + [y]
                    else:
                        line = list(samples[index]) + list(y)
            ...
            for i, y in enumerate(res):
                ...
                valid_outputs.append(y)

            df = pd.DataFrame(valid_samples, columns=None)
            df2 = pd.DataFrame(valid_outputs, columns=None)
