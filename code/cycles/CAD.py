...
from components.compressor import Compressor_LP, Compressor_HP, Compressor_other
...
def CAD(compressors_inputs, valves_inputs, HP_evaporator, external_fluid_LT_param, external_fluid_MT_param, external_fluid_HT_param, initial_guess, save_results = True, plot_figures = False) :
    ...
    compressor_LP = Compressor_other() #Compressor_LP()
    compressor_HP = Compressor_other() #Compressor_HP()
...
N_LP = np.arange(45,56, 5)
N_HP = 50

# Power of the compressors
P_LP = np.arange(2.5, 3.6, 0.5) * 1e3
P_HP = np.arange(4.5, 7.5, 0.5) * 1e3

compressor_inputs = {'N_LP': N_LP, 'N_HP': N_HP, 'P_LP': P_LP, 'P_HP': P_HP}

# Valve opening
z_LP = np.arange(35,46, 1)
z_HP = 20
...
for nLP, pLP, pHP, zLP in itertools.product(N_LP, P_LP, P_HP, z_LP):
    ...
    compressor_inputs = {'N_LP': nLP, 'N_HP': N_HP, 'P_LP': pLP, 'P_HP': pHP}
    valve_inputs = {'z_LP': zLP, 'z_HP': z_HP}

    CAD(compressor_inputs, valve_inputs, HP_evaporator, external_fluid_LT_param, external_fluid_MT_param, external_fluid_HT_param, initial_guess)
