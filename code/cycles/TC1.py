...
def run_TC1_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False, warnings=False):
    ...
    # Compressor
    TC1.Compressor = Compressor_2_param(cycle=TC1, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    TC1.Evaporator = HEX_Design(states_in=[TC1.state_8, TC1.state_3_prime], states_out=[TC1.state_3, TC1.state_4_prime], 
                                mdot=[TC1.mdot_wf_top, None], name="Evaporator", beta=beta, phi=phi, gamma=gamma,
    ...
    TC1.GasCooler = HEX_Design(states_in=[TC1.state_5_prime, TC1.state_5], states_out=[TC1.state_6_prime, TC1.state_7], 
                               mdot=[None, TC1.mdot_wf_top], name="Gas Cooler", beta=beta, phi=phi, gamma=gamma,
                               mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    ...
    if warnings and (TC1.state_5.p > 5.5e6) :
    ...
    if plot_results or save_results:
        ...
        TC1.Ts_diagram(n=100, plot=plot_results, save=save_results)
        TC1.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        TC1.energy_chart(plot=plot_results, save=save_results)
        TC1.exergy_chart(T0 = 293.15, p0 = 1e5, plot=plot_results, save=save_results)

        # Plot heat exchangers diagrams
        TC1.Evaporator._plot(save=save_results, name_cycle=TC1.name, plot=plot_results)
        TC1.GasCooler._plot(save=save_results, name_cycle=TC1.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    TC1.Evaporator.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC1.name)
    TC1.GasCooler.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC1.name)
    ...
    return np.log(TC1.Evaporator.A), np.log(TC1.GasCooler.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_TC1_cycle(sample={}, verbose=False, print_results=True, plot_results=True, save_results=True, warnings=True)