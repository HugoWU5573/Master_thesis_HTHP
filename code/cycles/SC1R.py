...
def run_SC1R_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False):
    ...
    T4_prime = sample.get("T4_prime", 40 + 273.15)  # Inlet temperature of the external fluid in the heat sink [K] (can be varied in the sensitivity analysis)
    ...
    # Compressor
    SC1R.Compressor = Compressor_2_param(cycle=SC1R, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    SC1R.Evaporator = HEX_Design(states_in=[SC1R.state_10, SC1R.state_1_prime], states_out=[SC1R.state_1, SC1R.state_2_prime], 
                                 mdot=[SC1R.mdot_wf_bottom, None], name="Evaporator", beta=beta, phi=phi, gamma=gamma,
    ...
    SC1R.Condenser = HEX_Design(states_in=[SC1R.state_4_prime, SC1R.state_3], states_out=[SC1R.state_3_prime, SC1R.state_8], 
                                mdot=[None, SC1R.mdot_wf_bottom], name="Condenser", beta=beta, phi=phi, gamma=gamma,
                                mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_cond = SC1R.Condenser.Compute_Pinch()
    SC1R.mdot_MT = SC1R.Condenser.mdot_c
    SC1R.Recuperator = HEX_Design(states_in=[SC1R.state_1, SC1R.state_8], states_out=[SC1R.state_2, SC1R.state_9], 
                                  mdot=[SC1R.mdot_wf_bottom, SC1R.mdot_wf_bottom], name = 'Recuperator', beta=beta, phi=phi, gamma=gamma,
    ...
    if plot_results or save_results:
        ...
        SC1R.Ts_diagram(n=100, plot=plot_results, save=save_results)
        SC1R.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        SC1R.energy_chart(plot=plot_results, save=save_results)
        SC1R.exergy_chart(T0=293.15, p0 = 1e5, plot=plot_results, save=save_results)

        # Plot heat exchangers diagrams
        SC1R.Evaporator._plot(save=save_results, name_cycle=SC1R.name, plot=plot_results)
        SC1R.Condenser._plot(save=save_results, name_cycle=SC1R.name, plot=plot_results)
        SC1R.Recuperator._plot(save=save_results, name_cycle=SC1R.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    SC1R.Evaporator.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC1R.name)
    SC1R.Condenser.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC1R.name)
    SC1R.Recuperator.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC1R.name)
    ...
    return np.log(SC1R.Evaporator.A), np.log(SC1R.Condenser.A), np.log(SC1R.Recuperator.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_SC1R_cycle(sample={}, verbose=False, print_results=True, plot_results=True, save_results=True)