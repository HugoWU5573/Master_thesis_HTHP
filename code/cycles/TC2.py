...
def run_TC2_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False, warnings=False):
    ...
    p3_prime = 3e5                                          # Inlet pressure of the external fluid in the MT heat sink [Pa]

    # Heat sink parameters
    external_fluid_HT = 'Water'                             # External fluid in the heat sink
    T5_prime = sample.get("T5_prime", 80 + 273.15)          # Inlet temperature of the external fluid in the heat sink [K]
    ...
    # Compressors
    TC2.Compressor_1 = Compressor_2_param(cycle=TC2, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    TC2.Compressor_2 = Compressor_2_param(cycle=TC2, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    def iterative_process(p_gess, T_7_current, T_sup_current_1, T_sup_current_3) :
        ...
        r = ratio_evaporators * (TC2.state_1.h - TC2.state_10.h)
        h3_evap = (r*TC2.state_3.h - TC2.state_8.h * (TC2.state_3.h - TC2.state_3_comp.h)) / (r + TC2.state_3_comp.h - TC2.state_3.h)
    ...
    TC2.Evaporator_LT = HEX_Design(states_in=[TC2.state_10, TC2.state_1_prime], states_out=[TC2.state_1, TC2.state_2_prime], 
                                   mdot = [TC2.mdot_wf_bottom, None], name="Evaporator_LT", beta=beta, phi=phi, gamma=gamma, 
    ...
    TC2.Evaporator_MT = HEX_Design(states_in=[TC2.state_8, TC2.state_3_prime], states_out=[TC2.state_3_evap, TC2.state_4_prime], 
                                   mdot = [TC2.mdot_wf_top - TC2.mdot_wf_bottom, None], name="Evaporator_MT", beta=beta, phi=phi, gamma=gamma,
                                   mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_evap_MT = TC2.Evaporator_MT.Compute_Pinch()
    TC2.mdot_MT = TC2.Evaporator_MT.mdot_h
    TC2.GasCooler = HEX_Design(states_in=[TC2.state_5_prime, TC2.state_5], states_out=[TC2.state_6_prime, TC2.state_7], 
                               mdot=[None, TC2.mdot_wf_top], name="Gas Cooler", beta=beta, phi=phi, gamma=gamma,
                               mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    ...
    if warnings and (TC2.state_5.p > 5.5e6) :
    ...
    if plot_results or save_results:
        ...
        TC2.Ts_diagram(n=100, plot=plot_results, save=save_results)
        TC2.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        TC2.energy_chart(plot=plot_results, save=save_results)
        TC2.exergy_chart(T0=293.15, p0 = 1e5, plot=plot_results, save=save_results)
        
        # Plot heat exchangers diagrams
        TC2.Evaporator_LT._plot(save=save_results, name_cycle=TC2.name, plot=plot_results)
        TC2.Evaporator_MT._plot(save=save_results, name_cycle=TC2.name, plot=plot_results)
        TC2.GasCooler._plot(save=save_results, name_cycle=TC2.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    TC2.Evaporator_LT.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2.name)
    TC2.Evaporator_MT.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2.name)
    TC2.GasCooler.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2.name)
    ...
    return np.log(TC2.Evaporator_LT.A), np.log(TC2.Evaporator_MT.A), np.log(TC2.GasCooler.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_TC2_cycle(sample={}, verbose=False, print_results=True, plot_results=True, save_results=True, warnings=True)