...
def run_TC1R_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False, warnings=False):
    ...
    T3_prime = sample.get("T3_prime", 45 + 273.15)  # Inlet temperature of the external fluid in the heat source [K] (can be varied in the sensitivity analysis)
    ...
    T5_prime = sample.get("T5_prime", 80 + 273.15)  # Inlet temperature of the external fluid in the heat sink [K] (can be varied in the sensitivity analysis)
    ...
    # Compressor
    TC1R.Compressor = Compressor_2_param(cycle=TC1R, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    p3_best = p_best[0] ; p5_best = p_best[1]
    ...
    TC1R.Evaporator = HEX_Design(states_in=[TC1R.state_8, TC1R.state_3_prime], states_out=[TC1R.state_3, TC1R.state_4_prime], 
                                 mdot=[TC1R.mdot_wf_top, None], name="Evaporator", beta=beta, phi=phi, gamma=gamma,
    ...
    TC1R.GasCooler = HEX_Design(states_in=[TC1R.state_5_prime, TC1R.state_5], states_out=[TC1R.state_6_prime, TC1R.state_6], 
                                mdot=[None, TC1R.mdot_wf_top], name="Gas Cooler", beta=beta, phi=phi, gamma=gamma,
                                mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_gas_cooler = TC1R.GasCooler.Compute_Pinch()
    TC1R.mdot_HT = TC1R.GasCooler.mdot_c
    TC1R.Recuperator = HEX_Design(states_in=[TC1R.state_3, TC1R.state_6], states_out=[TC1R.state_4, TC1R.state_7], 
                                  mdot=[TC1R.mdot_wf_top, TC1R.mdot_wf_top], name = 'Recuperator', beta=beta, phi=phi, gamma=gamma, 
    ...
    if warnings and (TC1R.state_5.p > 5.5e6) :
    ...
    if plot_results or save_results:
        ...
        TC1R.Ts_diagram(n=100, plot=plot_results, save=save_results)
        TC1R.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        TC1R.energy_chart(plot=plot_results, save=save_results)
        TC1R.exergy_chart(T0 = 293.15, p0 = 1e5, plot=plot_results, save=save_results)

        # Plot heat exchangers diagrams
        TC1R.Evaporator._plot(save=save_results, name_cycle=TC1R.name, plot=plot_results)
        TC1R.GasCooler._plot(save=save_results, name_cycle=TC1R.name, plot=plot_results)
        TC1R.Recuperator._plot(save=save_results, name_cycle=TC1R.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    TC1R.Evaporator.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC1R.name)
    TC1R.GasCooler.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC1R.name)
    TC1R.Recuperator.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC1R.name)
    ...
    return np.log(TC1R.Evaporator.A), np.log(TC1R.GasCooler.A), np.log(TC1R.Recuperator.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_TC1R_cycle(sample={}, verbose=False, print_results=True, plot_results=True, save_results=True, warnings=True)