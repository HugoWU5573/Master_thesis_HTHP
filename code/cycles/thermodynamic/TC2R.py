...
def run_TC2R_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False, warnings=False):
    ...
    alpha = sample.get("alpha", 0.5)    # alpha = Q_evap_MT / (Q_evap_LT + Q_evap_MT) [-]

    # Heat sources parameters

        # 1. LT source
    external_fluid_LT = 'Water'                             # External fluid in the LT heat source
    T1_prime = sample.get("T1_prime", 15 + 273.75)          # Inlet temperature of the external fluid in the LT heat source [K]
    ...
    T3_prime = sample.get("T3_prime", 45 + 273.15)          # Inlet temperature of the external fluid in the MT heat source [K]
    ...
    T5_prime = sample.get("T5_prime", 85 + 273.15)          # Inlet temperature of the external fluid in the heat sink [K]
    glide_HT = 35                                           # Temperature glide in the gas cooler [K]
    ...
    # Compressors
    TC2R.Compressor_1 = Compressor_2_param(cycle=TC2R, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    TC2R.Compressor_2 = Compressor_2_param(cycle=TC2R, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    def iterative_process(p_gess, T_6_current, T_sup_current_1, T_sup_current_3): 
        ...
        TC2R.Recuperator_MT = HEX_Design(states_in=[TC2R.state_3, TC2R.state_6], states_out=[None, None], 
                                        mdot=[None, None], name="Recuperator_MT", mode = "Non-Dimensional", 
                                        type = "Recuperator", epsilon=recuperator_effectiveness)
        TC2R.Recuperator_MT.Solve_Recuperator()
        TC2R.state_4 = TC2R.Recuperator_MT.state_out_c
        TC2R.state_7 = TC2R.Recuperator_MT.state_out_h
        ...
        TC2R.Recuperator_LT = HEX_Design(states_in=[TC2R.state_1, TC2R.state_7], states_out=[None, None], 
                                        mdot=[None, None], name="Recuperator_LT", mode = "Non-Dimensional", 
                                        type = "Recuperator", epsilon=recuperator_effectiveness)
        TC2R.Recuperator_LT.Solve_Recuperator()
        TC2R.state_2 = TC2R.Recuperator_LT.state_out_c
        TC2R.state_9 = TC2R.Recuperator_LT.state_out_h
    ...
    TC2R.Evaporator_LT = HEX_Design(states_in=[TC2R.state_10, TC2R.state_1_prime], states_out=[TC2R.state_1, TC2R.state_2_prime], 
                                    mdot = [TC2R.mdot_wf_bottom, None], name="Evaporator_LT", beta=beta, phi=phi, gamma=gamma,
    ...
    TC2R.Evaporator_MT = HEX_Design(states_in=[TC2R.state_8, TC2R.state_3_prime], states_out=[TC2R.state_3_evap, TC2R.state_4_prime],
                                    mdot = [TC2R.mdot_wf_top - TC2R.mdot_wf_bottom, None], name="Evaporator_MT", beta=beta, phi=phi, gamma=gamma,
                                    mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_evap_MT = TC2R.Evaporator_MT.Compute_Pinch()
    TC2R.mdot_MT = TC2R.Evaporator_MT.mdot_h
    TC2R.GasCooler = HEX_Design(states_in=[TC2R.state_5_prime, TC2R.state_5], states_out=[TC2R.state_6_prime, TC2R.state_6], 
                                mdot=[None, TC2R.mdot_wf_top], name="GasCooler", beta=beta, phi=phi, gamma=gamma,
                                mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_gas_cooler = TC2R.GasCooler.Compute_Pinch()
    TC2R.mdot_HT = TC2R.GasCooler.mdot_c
    TC2R.Recuperator_MT = HEX_Design(states_in=[TC2R.state_3, TC2R.state_6], states_out=[TC2R.state_4, TC2R.state_7], 
                                    mdot=[TC2R.mdot_wf_top, TC2R.mdot_wf_top], name="Recuperator_MT", beta=beta, phi=phi, gamma=gamma,
                                    mode="Dimensional", type="Recuperator", epsilon=recuperator_effectiveness, 
                                    model="ACK18", corr_factors=corr_factors)
    TC2R.Recuperator_MT.Solve_Recuperator()
    TC2R.Recuperator_LT = HEX_Design(states_in=[TC2R.state_1, TC2R.state_7], states_out=[TC2R.state_2, TC2R.state_9], 
                                    mdot=[TC2R.mdot_wf_bottom, TC2R.mdot_wf_bottom], name="Recuperator_LT", beta=beta, phi=phi, gamma=gamma,
                                    mode="Dimensional", type="Recuperator", epsilon=recuperator_effectiveness, 
                                    model="ACK18", corr_factors=corr_factors)
    TC2R.Recuperator_LT.Solve_Recuperator()
    ...
    if warnings and (TC2R.state_5.p > 5.5e6) :
    ...
    if plot_results or save_results:
        ...
        TC2R.transforms = [Transform('isobaric_mixing', '3_comp', '3_evap', None),
                        ...
                        Transform('hex', '3', '4',TC2R.Recuperator_MT, label_in_secondary='6', label_out_secondary='7'),
                        Transform('hex', '1', '2',TC2R.Recuperator_LT, label_in_secondary='7', label_out_secondary='9')]

        # Plot T-s diagram with saturation curve
        TC2R.Ts_diagram(n=100, plot=plot_results, save=save_results)
        TC2R.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        TC2R.energy_chart(plot=plot_results, save=save_results)
        TC2R.exergy_chart(T0=293.15, p0 = 1e5, plot=plot_results, save=save_results)

        # Plot heat exchangers diagrams
        TC2R.Evaporator_LT._plot(save=save_results, name_cycle=TC2R.name, plot=plot_results)
        TC2R.Evaporator_MT._plot(save=save_results, name_cycle=TC2R.name, plot=plot_results)
        TC2R.GasCooler._plot(save=save_results, name_cycle=TC2R.name, plot=plot_results)
        TC2R.Recuperator_LT._plot(save=save_results, name_cycle=TC2R.name, plot=plot_results)
        TC2R.Recuperator_MT._plot(save=save_results, name_cycle=TC2R.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    TC2R.Evaporator_LT.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2R.name)
    TC2R.Evaporator_MT.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2R.name)
    TC2R.GasCooler.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2R.name)
    TC2R.Recuperator_LT.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2R.name)
    TC2R.Recuperator_MT.Compute_Area(plot=plot_results, save=save_results, name_cycle=TC2R.name)
    ...
    if print_results:
        ...
        print(TC2R.Recuperator_LT)
        print(TC2R.Recuperator_MT)
    ...
    if save_results:
        ...
        with open(output_file, 'w') as f:
            ...
            f.write('\n' + str(TC2R.GasCooler) + '\n')
            f.write('\n' + str(TC2R.Recuperator_LT) + '\n')
            f.write('\n' + str(TC2R.Recuperator_MT) + '\n')

    return np.log(TC2R.Evaporator_LT.A), np.log(TC2R.Evaporator_MT.A), np.log(TC2R.GasCooler.A), np.log(TC2R.Recuperator_LT.A), np.log(TC2R.Recuperator_MT.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_TC2R_cycle(sample={}, verbose=False, print_results=True, plot_results=True, save_results=True, warnings=True)