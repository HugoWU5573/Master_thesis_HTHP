...
def run_SC2_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False):
    ...
    # Compressors
    SC2.Compressor_1 = Compressor_2_param(cycle=SC2, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    SC2.Compressor_2 = Compressor_2_param(cycle=SC2, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    if print_results:
        ...
        print(f"  - Superheating at first compressor inlet : {T_sup_best_1:.2f} K")
        print(f"  - Superheating at second compressor inlet : {T_sup_best_3:.2f} K")
    ...
    SC2.Evaporator_LT = HEX_Design(states_in=[SC2.state_10, SC2.state_1_prime], states_out=[SC2.state_1, SC2.state_2_prime], 
                                   mdot = [SC2.mdot_wf_bottom, None], name="Evaporator_LT", beta=beta, phi=phi, gamma=gamma, 
    ...
    SC2.Evaporator_MT = HEX_Design(states_in=[SC2.state_8, SC2.state_3_prime], states_out=[SC2.state_3_evap, SC2.state_4_prime],
                                   mdot = [SC2.mdot_wf_top - SC2.mdot_wf_bottom, None], name="Evaporator_MT", beta=beta, phi=phi, gamma=gamma,
                                   mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_evap_MT = SC2.Evaporator_MT.Compute_Pinch()
    SC2.mdot_MT = SC2.Evaporator_MT.mdot_h
    SC2.Condenser = HEX_Design(states_in=[SC2.state_5_prime, SC2.state_5], states_out=[SC2.state_6_prime, SC2.state_7], 
                               mdot=[None, SC2.mdot_wf_top], name="Condenser", beta=beta, phi=phi, gamma=gamma,
                               mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    ...
    if plot_results or save_results:
        ...
        SC2.Ts_diagram(n=100, plot=plot_results, save=save_results)
        SC2.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        SC2.energy_chart(plot=plot_results, save=save_results)
        SC2.exergy_chart(T0=293.15, p0 = 1e5, plot=plot_results, save=save_results)
        
        # Plot heat exchangers diagrams
        SC2.Evaporator_LT._plot(save=save_results, name_cycle=SC2.name, plot=plot_results)
        SC2.Evaporator_MT._plot(save=save_results, name_cycle=SC2.name, plot=plot_results)
        SC2.Condenser._plot(save=save_results, name_cycle=SC2.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    SC2.Evaporator_LT.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2.name)
    SC2.Evaporator_MT.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2.name)
    SC2.Condenser.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2.name)
    ...
    return np.log(SC2.Evaporator_LT.A), np.log(SC2.Evaporator_MT.A), np.log(SC2.Condenser.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_SC2_cycle(sample={}, verbose=False, print_results=True, plot_results=True, save_results=True)