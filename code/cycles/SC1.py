...
def run_SC1_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False):
    ...
    # Compressor
    SC1.Compressor = Compressor_2_param(cycle=SC1, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    SC1.Evaporator = HEX_Design(states_in=[SC1.state_10, SC1.state_1_prime], states_out=[SC1.state_1, SC1.state_2_prime], 
                                mdot=[SC1.mdot_wf_bottom, None], name="Evaporator", beta=beta, phi=phi, gamma=gamma,
    ...
    SC1.Condenser = HEX_Design(states_in=[SC1.state_4_prime, SC1.state_3], states_out=[SC1.state_3_prime, SC1.state_9], 
                               mdot=[None, SC1.mdot_wf_bottom], name="Condenser", beta=beta, phi=phi, gamma=gamma,
                               mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    ...
    if plot_results or save_results:
        ...
        SC1.Ts_diagram(n=100, plot=plot_results, save=save_results)
        SC1.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        SC1.energy_chart(plot=plot_results, save=save_results)
        SC1.exergy_chart(T0=293.15, p0 = 1e5, plot=plot_results, save=save_results)

        # Plot heat exchangers diagrams
        SC1.Evaporator._plot(save=save_results, name_cycle=SC1.name, plot=plot_results)
        SC1.Condenser._plot(save=save_results, name_cycle=SC1.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    SC1.Evaporator.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC1.name)
    SC1.Condenser.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC1.name)
    ...
    return np.log(SC1.Evaporator.A), np.log(SC1.Condenser.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_SC1_cycle({}, verbose=False, print_results=True, plot_results=True, save_results=True)