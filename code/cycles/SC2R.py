...
def run_SC2R_cycle(sample, verbose=False, print_results=False, plot_results=False, save_results=False):
    ...
    eta_is = sample.get("eta_is", 0.65)     # Isentropic efficiency of the compressor (can be varied in the sensitivity analysis)
    ...
    Q = sample.get("Q", 25000.0)      # Power output at the condenser [W] (can be varied in the sensitivity analysis)
    alpha = sample.get("alpha", 0.5)  # alpha = Q_evap_MT / (Q_evap_LT + Q_evap_MT) [-] (can be varied in the sensitivity analysis)

    # Heat sources parameters

        # 1. LT source
    external_fluid_LT = 'Water'                     # External fluid in the heat source
    T1_prime = sample.get("T1_prime", 15 + 273.75)  # Inlet temperature of the external fluid in the heat source [K] (can be varied in the sensitivity analysis)
    ...
    T3_prime = sample.get("T3_prime", 40 + 273.15)  # Inlet temperature of the external fluid in the heat sink [K] (can be varied in the sensitivity analysis)
    ...
    T5_prime = sample.get("T5_prime", 60 + 273.15)  # Inlet temperature of the external fluid in the heat sink [K] (can be varied in the sensitivity analysis)
    ...
    corr_factors = {
        "h_single_phase": sample.get("h_single_phase", 1.0),
        "h_evaporation": sample.get("h_evaporation", 1.0),
        "h_condensation": sample.get("h_condensation", 1.0),
        "h_supercritical": sample.get("h_supercritical", 1.0),
        "f_single_phase": sample.get("f_single_phase", 1.0),
        "f_evaporation": sample.get("f_evaporation", 1.0),
        "f_condensation": sample.get("f_condensation", 1.0),
        "f_supercritical": sample.get("f_supercritical", 1.0),
    }

    # Geometric parameters of the BPHEs (can be varied in the sensitivity analysis)
    beta = sample.get("beta", 45.0)
    phi   = sample.get("phi", 1.2)
    gamma = sample.get("gamma", 0.55)
    ...
    # Compressors
    SC2R.Compressor_1 = Compressor_2_param(cycle=SC2R, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    SC2R.Compressor_2 = Compressor_2_param(cycle=SC2R, eta_v=eta_v, eta_is_max=eta_is, fluid=working_fluid, eta_elme=eta_elme)
    ...
    def iterative_process(p_gess, T_sub_current, T_sup_current_1, T_sup_current_3): 
        ...
        SC2R.Recuperator_MT = HEX_Design(states_in=[SC2R.state_3, SC2R.state_6], states_out=[None, None], 
                                        mdot=[None, None], name="Recuperator_MT", mode = "Non-Dimensional", 
                                        type = "Recuperator", epsilon=recuperator_effectiveness)
        SC2R.Recuperator_MT.Solve_Recuperator()
        SC2R.state_4 = SC2R.Recuperator_MT.state_out_c
        SC2R.state_7 = SC2R.Recuperator_MT.state_out_h
        ...
        SC2R.Recuperator_LT = HEX_Design(states_in=[SC2R.state_1, SC2R.state_7], states_out=[None, None], 
                                        mdot=[None, None], name="Recuperator_LT", mode = "Non-Dimensional", 
                                        type = "Recuperator", epsilon=recuperator_effectiveness)
        SC2R.Recuperator_LT.Solve_Recuperator()
        SC2R.state_2 = SC2R.Recuperator_LT.state_out_c
        SC2R.state_9 = SC2R.Recuperator_LT.state_out_h
    ...
    SC2R.Evaporator_LT = HEX_Design(states_in=[SC2R.state_10, SC2R.state_1_prime], states_out=[SC2R.state_1, SC2R.state_2_prime], 
                                    mdot = [SC2R.mdot_wf_bottom, None], name="Evaporator_LT", beta=beta, phi=phi, gamma=gamma,
    ...
    SC2R.Evaporator_MT = HEX_Design(states_in=[SC2R.state_8, SC2R.state_3_prime], states_out=[SC2R.state_3_evap, SC2R.state_4_prime],
                                    mdot = [SC2R.mdot_wf_top - SC2R.mdot_wf_bottom, None], name="Evaporator_MT", beta=beta, phi=phi, gamma=gamma,
                                    mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_evap_MT = SC2R.Evaporator_MT.Compute_Pinch()
    SC2R.mdot_MT = SC2R.Evaporator_MT.mdot_h
    SC2R.Condenser = HEX_Design(states_in=[SC2R.state_5_prime, SC2R.state_5], states_out=[SC2R.state_6_prime, SC2R.state_6], 
                                mdot=[None, SC2R.mdot_wf_top], name="Condenser", beta=beta, phi=phi, gamma=gamma,
                                mode="Dimensional", model="ACP70X", corr_factors=corr_factors)
    T_pinch_cond = SC2R.Condenser.Compute_Pinch()
    SC2R.mdot_HT = SC2R.Condenser.mdot_c
    SC2R.Recuperator_MT = HEX_Design(states_in=[SC2R.state_3, SC2R.state_6], states_out=[SC2R.state_4, SC2R.state_7], 
                                    mdot=[SC2R.mdot_wf_top, SC2R.mdot_wf_top], name="Recuperator_MT", beta=beta, phi=phi, gamma=gamma,
                                    mode="Dimensional", type="Recuperator", epsilon=recuperator_effectiveness, 
                                    model="ACK18", corr_factors=corr_factors)
    SC2R.Recuperator_MT.Solve_Recuperator()
    SC2R.Recuperator_LT = HEX_Design(states_in=[SC2R.state_1, SC2R.state_7], states_out=[SC2R.state_2, SC2R.state_9], 
                                    mdot=[SC2R.mdot_wf_bottom, SC2R.mdot_wf_bottom], name="Recuperator_LT", beta=beta, phi=phi, gamma=gamma,
                                    mode="Dimensional", type="Recuperator", epsilon=recuperator_effectiveness, 
                                    model="ACK18", corr_factors=corr_factors)
    SC2R.Recuperator_LT.Solve_Recuperator()
    ...
    if plot_results or save_results:
        ...
        SC2R.transforms = [Transform('isobaric_mixing', '3_comp', '3_evap', None),
                        ...
                        Transform('hex', '3', '4',SC2R.Recuperator_MT, label_in_secondary='6', label_out_secondary='7'),
                        Transform('hex', '1', '2',SC2R.Recuperator_LT, label_in_secondary='7', label_out_secondary='9')]

        # Plot T-s diagram with saturation curve
        SC2R.Ts_diagram(n=100, plot=plot_results, save=save_results)
        SC2R.ph_diagram(n=100, plot=plot_results, save=save_results)

        # Plot energy and exergy charts
        SC2R.energy_chart(plot=plot_results, save=save_results)
        SC2R.exergy_chart(T0=293.15, p0 = 1e5, plot=plot_results, save=save_results)

        # Plot heat exchangers diagrams
        SC2R.Evaporator_LT._plot(save=save_results, name_cycle=SC2R.name, plot=plot_results)
        SC2R.Evaporator_MT._plot(save=save_results, name_cycle=SC2R.name, plot=plot_results)
        SC2R.Condenser._plot(save=save_results, name_cycle=SC2R.name, plot=plot_results)
        SC2R.Recuperator_LT._plot(save=save_results, name_cycle=SC2R.name, plot=plot_results)
        SC2R.Recuperator_MT._plot(save=save_results, name_cycle=SC2R.name, plot=plot_results)

    # Compute the HEX areas and plot the design criteria
    SC2R.Evaporator_LT.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2R.name)
    SC2R.Evaporator_MT.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2R.name)
    SC2R.Condenser.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2R.name)
    SC2R.Recuperator_LT.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2R.name)
    SC2R.Recuperator_MT.Compute_Area(plot=plot_results, save=save_results, name_cycle=SC2R.name)
    ...
    if print_results:
        ...
        print(SC2R.Recuperator_LT)
        print(SC2R.Recuperator_MT)
    ...
    if save_results:
        ...
        with open(output_file, 'w') as f:
            ...
            f.write('\n' + str(SC2R.Condenser) + '\n')
            f.write('\n' + str(SC2R.Recuperator_LT) + '\n')
            f.write('\n' + str(SC2R.Recuperator_MT) + '\n')


    return np.log(SC2R.Evaporator_LT.A), np.log(SC2R.Evaporator_MT.A), np.log(SC2R.Condenser.A), np.log(SC2R.Recuperator_LT.A), np.log(SC2R.Recuperator_MT.A)


if __name__ == "__main__":

    # We run the cycle with the default parameters (nominal conditions) -> sample is an empty dictionary
    run_SC2R_cycle(sample={}, verbose=False, print_results=True, plot_results=True, save_results=True)
