...
error_P_max_rel = np.nanmax(abs(P_2_calc.flatten() - P.flatten())/P.flatten())
errors_P = abs(P_2_calc.flatten() - P.flatten())/P.flatten()
errors_P_90th = np.nanpercentile(errors_P, 90)
print(f'90th percentile of P errors: {errors_P_90th*100:.2f} %')

P_linspace = np.linspace(1e3, 6e3, 100)
plt.figure()
plt.plot(P.flatten()/1e3, P_2_calc.flatten()/1e3, 'kx', label = 'Calculated', clip_on=False)
plt.plot(P_linspace/1e3, P_linspace/1e3, 'k-', label='Datasheet', clip_on=False)
plt.fill_between(P_linspace/1e3, (P_linspace) * (1 - error_P_max_rel)/1e3, (P_linspace) * (1 + error_P_max_rel)/1e3, color = 'orange', alpha=0.3, label='Error Range')
plt.fill_between(P_linspace/1e3, (P_linspace) * (1 - errors_P_90th)/1e3, (P_linspace) * (1 + errors_P_90th)/1e3, color = 'red', alpha=0.3, label='90th Percentile Error Range')

plt.text(P_linspace[len(P_linspace)//2]/1e3, (P_linspace[len(P_linspace)//2]/1e3) - 1.3, f'{error_P_max_rel*100:.1f}% error', fontsize=12)
plt.text(P_linspace[len(P_linspace)//2]/1e3, (P_linspace[len(P_linspace)//2]/1e3) - 1, f'{errors_P_90th*100:.1f}% error (90th percentile)', fontsize=12)
plt.title(r'$P_{\mathrm{calc}}$ [kW]', loc='left', fontsize=12)
plt.xlabel(r'$P_{\mathrm{data}}$ [kW]', fontsize=12)
...
plt.tight_layout()
plt.savefig('code/fitting/compressor/error_P_LP.png', dpi=300)
#plt.show()

plt.figure()
error_mdot_max = np.nanmax(abs(mdot_calc.flatten() - mdot.flatten()))
error_mdot_max_rel = np.nanmax(abs(mdot_calc.flatten() - mdot.flatten())/mdot.flatten())
mdot_linspace = np.linspace(0.02, 0.08, 100)
plt.plot(mdot.flatten(), mdot_calc.flatten(), 'kx', label='Calculated', clip_on=False)
plt.plot(mdot_linspace, mdot_linspace, 'k-', label='Datasheet', clip_on=False)
plt.fill_between(mdot_linspace, mdot_linspace * (1 - error_mdot_max_rel), mdot_linspace * (1 + error_mdot_max_rel), color = 'orange', alpha=0.3, label='Error Range')
plt.text(mdot_linspace[len(mdot_linspace)//2], mdot_linspace[len(mdot_linspace)//2] - error_mdot_max - 0.005, f'{error_mdot_max_rel*100:.1f}% error', fontsize=12)
plt.title(r'$\dot{m}_{\mathrm{calc}}$ [kg/s]', loc='left', fontsize=12)
...
#plt.legend(frameon=False, loc='best', fontsize=12)
plt.tight_layout()
plt.savefig('code/fitting/compressor/error_mdot_LP.png', dpi=300)
#plt.show()

error_T_max = np.nanmax(abs(T_2_calc.flatten() - T_2.flatten()))
linspace_T = np.linspace(50 + 273.15, 90 + 273.15, 100)
plt.figure()
plt.plot(T_2.flatten() - 273.15, T_2_calc.flatten() - 273.15, 'kx', label='Calculated', clip_on=False)
plt.plot(linspace_T - 273.15, linspace_T - 273.15, 'k-', label='Datasheet', clip_on=False)
...
plt.text(linspace_T[len(linspace_T)//2] - 273.15, linspace_T[len(linspace_T)//2] - error_T_max - 273.15 - 5, f'{error_T_max:.1f}K error', fontsize=12)
plt.title(r'$T_{\mathrm{calc}}$ [°C]', loc='left', fontsize=12)
plt.xlabel(r'$T_{\mathrm{data}}$ [°C]', fontsize=12)
# Hide top and right spines
ax = plt.gca()
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
...
plt.legend(frameon=False, loc='best', fontsize=12)
plt.tight_layout()
plt.savefig('code/fitting/compressor/error_T_LP.png', dpi=300)
#plt.show()
...
plt.xlabel(r'$p_{data}$ [bar]', fontsize=12)
...
plt.xlabel(r'$T_{data}$ [°C]', fontsize=12)
# Add some text for labels, title and custom x-axis tick labels, etc.
ax = plt.gca()
ax.tick_params(axis='both', which='major')
...
error_P_max = np.nanmax(abs(P_2_calc.flatten() - P.flatten()))
P_linspace = np.linspace(5e3, 7.5e3, 100)
error_P_max_rel = np.nanmax(abs(P_2_calc.flatten() - P.flatten())/P.flatten())
plt.figure()
plt.plot(P.flatten()/1e3, P_2_calc.flatten()/1e3, 'kx', label = 'Calculated', clip_on=False)
plt.plot(P_linspace/1e3, P_linspace/1e3, 'k-', label='Datasheet', clip_on=False)
plt.fill_between(P_linspace/1e3, P_linspace/1e3 - error_P_max_rel*P_linspace/1e3, P_linspace/1e3 + error_P_max_rel*P_linspace/1e3, color = 'orange', alpha=0.3, label='Error Range')

plt.text(P_linspace[len(P_linspace)//2]/1e3, (P_linspace[len(P_linspace)//2] - error_P_max)/1e3 - 0.5, f'{error_P_max_rel*100:.1f}% error', fontsize=12)
plt.title(r'$P_{\mathrm{calc}}$ [kW]', loc='left', fontsize=12)
plt.xlabel(r'$P_{\mathrm{data}}$ [kW]', fontsize=12)
# Hide top and right spines
ax = plt.gca()
...
plt.tick_params(axis='x', rotation=0)
plt.tick_params(axis='both', which='major', labelsize=11, direction='in')
#ax.set_xticks(np.arange(5, 7.5, 0.2))
plt.xlim(5, 7.5)
#ax.set_yticks(np.arange(5.2, 7.4, 0.2))
plt.ylim(5, 7.5)
#plt.axis('equal')
#plt.legend(frameon=False, loc='best', fontsize=12)
plt.tight_layout()
plt.savefig('code/fitting/compressor/error_P_HP.png', dpi=300)
#plt.show()

plt.figure()
error_mdot_max = np.nanmax(abs(mdot_calc.flatten() - mdot.flatten()))
mdot_linspace = np.linspace(0.04, 0.11, 100)
error_mdot_max_rel = np.nanmax(abs(mdot_calc.flatten() - mdot.flatten())/mdot.flatten())
plt.plot(mdot.flatten(), mdot_calc.flatten(), 'kx', label='Calculated', clip_on=False)
plt.plot(mdot_linspace, mdot_linspace, 'k-', label='Datasheet', clip_on=False)
plt.fill_between(mdot_linspace, mdot_linspace - error_mdot_max_rel*mdot_linspace, mdot_linspace + error_mdot_max_rel*mdot_linspace, color = 'orange', alpha=0.3, label='Error Range')
plt.text(mdot_linspace[len(mdot_linspace)//2], mdot_linspace[len(mdot_linspace)//2] - error_mdot_max - 0.005, f'{error_mdot_max_rel*100:.1f}% error', fontsize=12)
plt.title(r'$\dot{m}_{\mathrm{calc}}$ [kg/s]', loc='left', fontsize=12)
plt.xlabel(r'$\dot{m}_{\mathrm{data}}$ [kg/s]', fontsize=12)
# Hide top and right spines
...
#plt.legend(frameon=False, loc='best', fontsize=12)
plt.tight_layout()
plt.savefig('code/fitting/compressor/error_mdot_HP.png', dpi=300)
#plt.show()

error_T_max = np.nanmax(abs(T_2_calc.flatten() - T_2.flatten()))
linspace_T = np.linspace(110 + 273.15, 145 + 273.15, 100)
plt.figure()
plt.plot(T_2.flatten() - 273.15, T_2_calc.flatten() - 273.15, 'kx', label='Calculated', clip_on=False)
plt.plot(linspace_T - 273.15, linspace_T - 273.15, 'k-', label='Datasheet', clip_on=False)
plt.fill_between(linspace_T - 273.15, linspace_T + error_T_max - 273.15, linspace_T - error_T_max - 273.15, color = 'orange', alpha=0.3, label='Error Range')
...
#plt.legend(frameon=False, loc='best', fontsize=12)
plt.tight_layout()
plt.savefig('code/fitting/compressor/error_T_HP.png', dpi=300)
#plt.show()
